...
struct tok {
  ...
  enum { PRIM = 0, LPAR, RPAR, NOT, AND, OR, END } type;
};
...
struct execarg {
  union {
    struct {
      char ***
          braces; /* NULL terminated list of pointers into argv where {} were */
    } s;          /* semicolon */
    struct {
...
static struct op_info ops[] = {
    ...
    {")", RPAR, 0, 0, 0}, {"!", NOT, 3, 1, 0},
    {"-a", AND, 2, 2, 1}, {"-o", OR, 1, 2, 1},
...
static struct tok
    *toks; /* holds allocated array of all toks created while parsing */
static struct tok
    *root; /* points to root of expression tree, inside toks array */

static struct timespec start; /* time find was started, used for -[acm]time */

static size_t
    envlen; /* number of bytes in environ, used to calculate against ARG_MAX */
...
static int pri_nouser(struct arg *arg) { return !getpwuid(arg->st->st_uid); }

static int pri_nogroup(struct arg *arg) { return !getgrgid(arg->st->st_gid); }

static int pri_xdev(struct arg *arg) { return 1; }

static int pri_prune(struct arg *arg) { return gflags.prune = 1; }
...
static int pri_type(struct arg *arg) {
  ...
  default:
    return 0; /* impossible, but placate warnings */
  case 'b':
    return S_ISBLK(arg->st->st_mode);
  case 'c':
    return S_ISCHR(arg->st->st_mode);
  case 'd':
    return S_ISDIR(arg->st->st_mode);
  case 'l':
    return S_ISLNK(arg->st->st_mode);
  case 'p':
    return S_ISFIFO(arg->st->st_mode);
  case 'f':
    return S_ISREG(arg->st->st_mode);
  case 's':
    return S_ISSOCK(arg->st->st_mode);
  }
}
...
static int pri_depth(struct arg *arg) { return 1; }
  ...
  *arg = NULL;
  ...
  if (e->isplus) {
    *(arg - 1) = NULL; /* don't need the {} in there now */
    ...
    e->argv = argv;
    e->u.s.braces = ereallocarray(NULL, ++nbraces,
                                  sizeof(*e->u.s.braces)); /* ++ for NULL */
 ...
 * Freeargs
 */
static void free_extra(union extra extra) { free(extra.p); }
...
static void parse(int argc, char **argv) {
  ...
  for (arg = argv, tok = infix; *arg; arg++, tok++) {
    ...
    if (pri) { /* token is a primary, fill out tok and get arguments */
      ...
      if ((lasttype == PRIM || lasttype == RPAR) &&
          (op->type == NOT ||
           op->type == LPAR)) { /* need another implicit -a */
        *tok++ = and;
        ntok++;
      }
      tok->type = op->type;
  ...
  if (gflags.print &&
      (arg != argv)) /* need to add -a -print (not just -print) */
    gflags.print++;
  ...
  for (tok = infix, out = rpn, top = stack; tok->type != END; tok++) {
    ...
    default:
       ...
       * what's going on */
      while (top-- > stack && ((tok->u.oinfo->lassoc &&
                                tok->u.oinfo->prec <= (*top)->u.oinfo->prec) ||
                               (!tok->u.oinfo->lassoc &&
                                tok->u.oinfo->prec < (*top)->u.oinfo->prec)))
        *out++ = **top;
...
static int eval(struct tok *tok, struct arg *arg) {
  ...
  if ((tok->type == AND && ret) || (tok->type == OR && !ret) ||
      tok->type == NOT)
...
static void find(char *path, struct findhist *hist) {
  ...
  if (!S_ISDIR(st.st_mode) || !gflags.depth ||
      (gflags.xdev && hist && st.st_dev != hist->dev))
    eval(root, &arg);

  if (!S_ISDIR(st.st_mode) || gflags.prune ||
      (gflags.xdev && hist && st.st_dev != hist->dev))
    return;
...
int find_main(int argc, char **argv) {
  ...
  for (; *argv && **argv != '-' && strcmp(*argv, "!") && strcmp(*argv, "(");
       argv++)
    ;
