...
void getlines(FILE *fp, struct linebuf *b) {
  char *line = NULL;
  ...
  while ((len = getline(&line, &size, fp)) > 0) {
    ...
    b->lines[b->nlines - 1].data =
        memcpy(emalloc(linelen + 1), line, linelen + 1);
    b->lines[b->nlines - 1].len = linelen;
  }
  free(line);
  if (b->lines && b->nlines && linelen &&
      b->lines[b->nlines - 1].data[linelen - 1] != '\n') {
    b->lines[b->nlines - 1].data =
        erealloc(b->lines[b->nlines - 1].data, linelen + 2);
