...
static int mdcheckline(const char *s, uint8_t *md, size_t sz) {
  size_t i;
  int b1, b2;
...
static void mdchecklist(FILE *listfp, struct crypt_ops *ops, uint8_t *md,
                        size_t sz, int *formatsucks, int *noread,
                        int *nonmatch) {
  int fd;
  size_t bufsiz = 0;
  int r;
...
int cryptcheck(int argc, char *argv[], struct crypt_ops *ops, uint8_t *md,
               size_t sz) {
  FILE *fp;
...
int cryptmain(int argc, char *argv[], struct crypt_ops *ops, uint8_t *md,
              size_t sz) {
  int fd;
  int ret = 0;
...
int cryptsum(struct crypt_ops *ops, int fd, const char *f, uint8_t *md) {
  uint8_t buf[BUFSIZ];
  ssize_t n;
